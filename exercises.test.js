...
test('Question 7: searchWord', () => {
  ...
  expect(exercises.searchWord('his', text)).toBe(4); // 4
  expect(exercises.searchWord('hip-hop', text)).toBe(1); // 1 (because "hip-hop's" !== "hip-hop")
  expect(exercises.searchWord('flow', text)).toBe(1); 
  expect(exercises.searchWord('is', text)).toBe(1); 
})